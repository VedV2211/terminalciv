...
while choice != "7":
    ...
    # This is a menu where the player can buy new factories
        ...
        factorychoice = input(
            "    Which factory would you like to buy? \n    1. The farm costs "
            + str(farmCost)
            + " coins. You will have "
            + str(farms + 1)
            + " farms. \n    2. The mill costs "
            ...
            + " mines. \n    4. The barrack costs "
            + str(barrackCost)
            + " food, wood and metal. You will have "
            + str(barracks + 1)
        ...
        if (factory == 1) & (coins >= ((farmCost * nOF) + ((nOF**2) - nOF) / 2)):
            ...
            (factory == 4)
            & (food >= ((barrackCost * nOF) + ((nOF**2) - nOF) / 2))
            & (wood >= ((barrackCost * nOF) + ((nOF**2) - nOF) / 2))
            & (metal >= ((barrackCost * nOF) + ((nOF**2) - nOF) / 2))
        ):
            for i in range(nOF):
                barracks += 1
                food -= barrackCost
        ...
        while soldiers > 0:
            ...
            if enemyHealth[battleStage] > 0:
                if enemyAttackchoice == 1:
                    enemyAttack = (random.randint(20, 60)) * (
                        (enemyStrength[battleStage]) ** 2
                    )
                    ...
                    enemyHealed = (random.randint(5, 20)) * (
                        enemyStrength[battleStage] ** 2
                    )
                    print(
                        "    The enemy used their medical supplies and healed "
                        + str(enemyHealed)
                        + " health points. "
                    )
    ...
    # The user can wait for a certain number of years (seconds) which gains resources
        ...
        for i in range(years):
            ...
            if (coins < 0) or (food < 0) or (wood < 0) or (metal < 0):
                print(
                    "You have run out of resources, and your kingdom is on the brink of perishing. "
                )
                break
        print("")
    ...
    # The user can get basic help
        ...
        print(
            "Choose a number for your choice unless in battle, where you choose to attack or defend. "
        )
